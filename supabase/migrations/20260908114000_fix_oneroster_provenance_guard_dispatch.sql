-- Dispatch by table before referencing row fields unique to that table.
...
BEGIN
  ...
  IF TG_TABLE_NAME = 'academic_terms' THEN
    IF NEW.term_name IS DISTINCT FROM OLD.term_name
      ...
      RAISE EXCEPTION 'Externally managed term fields cannot be edited locally'
        USING ERRCODE = '42501';
    END IF;
  ELSIF TG_TABLE_NAME = 'course_blueprints' THEN
