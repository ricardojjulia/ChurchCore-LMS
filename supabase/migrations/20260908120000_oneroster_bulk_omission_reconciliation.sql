-- Support OneRoster bulk manifests for academic data by reconciling omissions.
-- Bulk remains intentionally limited to academicSessions, courses, and classes;
-- identity, roles, and enrollments still require the later linking phase.
...
DECLARE
  ...
  v_bulk_academic_sessions boolean := false;
  v_bulk_courses boolean := false;
  v_bulk_classes boolean := false;
BEGIN
  IF NOT EXISTS (
  ...
  SELECT EXISTS (
    SELECT 1 FROM public.oneroster_import_rows
    WHERE job_id = p_job_id AND org_id = p_org_id AND file_type = 'manifest'
      AND normalized_payload->>'propertyName' = 'file.academicSessions'
      AND normalized_payload->>'value' = 'bulk'
  ) INTO v_bulk_academic_sessions;
  SELECT EXISTS (
    SELECT 1 FROM public.oneroster_import_rows
    WHERE job_id = p_job_id AND org_id = p_org_id AND file_type = 'manifest'
      AND normalized_payload->>'propertyName' = 'file.courses'
      AND normalized_payload->>'value' = 'bulk'
  ) INTO v_bulk_courses;
  SELECT EXISTS (
    SELECT 1 FROM public.oneroster_import_rows
    WHERE job_id = p_job_id AND org_id = p_org_id AND file_type = 'manifest'
      AND normalized_payload->>'propertyName' = 'file.classes'
      AND normalized_payload->>'value' = 'bulk'
  ) INTO v_bulk_classes;
  ...
  IF v_bulk_classes THEN
    SELECT count(*) INTO v_count
    FROM public.external_entity_links l
    JOIN public.course_sections s ON s.id = l.local_id AND s.org_id = p_org_id AND s.is_active
    WHERE l.org_id = p_org_id AND l.source_system = v_job.source_system
      ...
      AND l.object_type = 'class' AND l.local_table = 'course_sections'
      AND NOT EXISTS (
        SELECT 1 FROM public.oneroster_import_rows r
        WHERE r.job_id = p_job_id AND r.org_id = p_org_id
          AND r.file_type = 'classes' AND r.sourced_id = l.sourced_id
      );
    v_deactivated := v_deactivated + v_count;
  END IF;

  IF v_bulk_courses THEN
    SELECT count(*) INTO v_count
    FROM public.external_entity_links l
    JOIN public.course_blueprints b ON b.id = l.local_id AND b.org_id = p_org_id AND b.is_active
    WHERE l.org_id = p_org_id AND l.source_system = v_job.source_system
      AND l.source_tenant_id IS NOT DISTINCT FROM v_job.source_tenant_id
      AND l.object_type = 'course' AND l.local_table = 'course_blueprints'
      AND NOT EXISTS (
        SELECT 1 FROM public.oneroster_import_rows r
        WHERE r.job_id = p_job_id AND r.org_id = p_org_id
          AND r.file_type = 'courses' AND r.sourced_id = l.sourced_id
      );
    v_deactivated := v_deactivated + v_count;
  END IF;

  IF v_bulk_academic_sessions THEN
    SELECT count(*) INTO v_count
    FROM public.external_entity_links l
    JOIN public.academic_terms t ON t.id = l.local_id AND t.org_id = p_org_id AND t.is_active
    WHERE l.org_id = p_org_id AND l.source_system = v_job.source_system
      AND l.source_tenant_id IS NOT DISTINCT FROM v_job.source_tenant_id
      AND l.object_type = 'academic_session' AND l.local_table = 'academic_terms'
      AND NOT EXISTS (
        SELECT 1 FROM public.oneroster_import_rows r
        WHERE r.job_id = p_job_id AND r.org_id = p_org_id
          AND r.file_type = 'academicSessions' AND r.sourced_id = l.sourced_id
      );
    v_deactivated := v_deactivated + v_count;
  END IF;
...
BEGIN
  ...
  IF v_bulk_classes THEN
    WITH omitted AS (
      SELECT l.id AS link_id, l.local_id
      FROM public.external_entity_links l
      JOIN public.course_sections s ON s.id = l.local_id AND s.org_id = p_org_id AND s.is_active
      WHERE l.org_id = p_org_id AND l.source_system = v_job.source_system
        AND l.source_tenant_id IS NOT DISTINCT FROM v_job.source_tenant_id
      ...
      UPDATE public.course_sections s SET is_active = false
      FROM omitted o WHERE s.id = o.local_id AND s.org_id = p_org_id AND s.is_active
      RETURNING o.link_id
    ), link_updates AS (
      UPDATE public.external_entity_links l SET source_status = 'inactive', updated_at = now()
      WHERE l.id IN (SELECT link_id FROM target_updates)
      RETURNING 1
    )
    SELECT count(*) INTO v_count FROM link_updates;
    v_deactivated := v_deactivated + v_count;
  END IF;
  ...
  IF v_bulk_courses THEN
    WITH omitted AS (
      ...
      UPDATE public.course_blueprints b SET is_active = false
      FROM omitted o WHERE b.id = o.local_id AND b.org_id = p_org_id AND b.is_active
      RETURNING o.link_id
    ), link_updates AS (
      UPDATE public.external_entity_links l SET source_status = 'inactive', updated_at = now()
      WHERE l.id IN (SELECT link_id FROM target_updates)
  ...
  IF v_bulk_academic_sessions THEN
    WITH omitted AS (
      ...
      UPDATE public.academic_terms t SET is_active = false
      FROM omitted o WHERE t.id = o.local_id AND t.org_id = p_org_id AND t.is_active
      RETURNING o.link_id
    ), link_updates AS (
      UPDATE public.external_entity_links l SET source_status = 'inactive', updated_at = now()
      WHERE l.id IN (SELECT link_id FROM target_updates)
